...
class System
{
	...
	function _initialize($api=false)
	{
		...
		if(!$api){
			...
			if($token = \JWTAuth::getToken() and $user = \JWTAuth::parseToken()->authenticate() and $user = User::find($user->id)){
				$user->load('roles.permissions');
			} else
			{
				$user = \Auth::user();
		...
		foreach($roles as $key => $role){
			foreach($role->permissions as $permission){
				if(array_key_exists($permission->slug,$this->roles)){
					$this->roles[$permission->slug]=$permission->pivot->permission == null ? 
						$permission->_default : 
	...
	function getCurrentMenuWidget($position=null)
	{
		...
		if(is_null($this->widgets))
		{
            ...
            $query = "SELECT w.id, w.title, wd.path,wd.slug, w.widget_id,w.configuration,w.position from widgetizes as w JOIN widgets as wd ON wd.id=w.widget_id JOIN menu_widgetize as wm ON w.id=wm.widgetize_id WHERE w.enabled=1 AND (wm.menu_id = {$this->current} or wm.menu_id=0) order by w.ordering DESC";
