...
class AccessController extends Controller
{
    ...
    {	
        ...
        \Admin::add_to_toolbar(['name'=>'Create role','href'=>route('admin.access.create.role')]);
    	$roles = Role::get();
    ...
    public function roleCreate(Request $request)
    {
    	$permissions = Permission::get();
        \Admin::add_to_toolbar(['name'=>'Frontpage','href'=>route('admin.frontpage.index')]);
        \Admin::add_to_toolbar(['name'=>'Go back','href'=>route('admin.access.index')]);
    ...
    public function doRole(Request $request,$role)
    {
    	...
    	if ($role = Role::with('permissions')->where('slug',$role)->first() ){
            $called = collect($request->permission);
			$permissions = Permission::whereIn('slug',array_keys($called->toArray()))->get();
			$array = [];
			foreach($permissions as $permission){
                if(array_key_exists($permission->slug,$request->permission)):
                    if($request->permission[$permission->slug] !== 'inherit'){
                    $array[$permission->id] = [
                        'permission' => $request->permission[$permission->slug] 
                    ];
                }
                endif;
				$requested = $called->where('name',$permission->slug)->first();
             ...
             // dd($request->permission,$requested,$permission->slug,$array);
			if(count($array)){
				$role->permissions()->sync($array);
            }
		}
