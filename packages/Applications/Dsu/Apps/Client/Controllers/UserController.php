...
class UserController extends Controller
{
	public function profile(Request $request)
	{
		...
		if($user = User::with('documents')->findorFail(Auth::id())){
			return view("DsuView::client.user.profile",compact('user'));
		}
	}
	...
	{
		if($documents = Document::get()){
			$user = User::findorFail(Auth::id());
			return view("DsuView::client.user.document",compact('documents','user'));
		}
	}
	...
	public function uploadDocuments(Request $request)
	{
		if($files = $request->file('document')){
			$messages = [
			    'mimes' => 'Document must be of one of the following mime type : jpeg,bmp,png,pdf',
			];
			$validator = \Validator::make($request->all(), [
			    'document.*' => 'mimes:jpeg,bmp,png,pdf',
			],$messages);
			if($validator->fails()){
				// dd($validator->errors());
				return redirect()->back()->withErrors($validator)->withInput();
			...
			$documents = Document::whereIn('slug',array_keys($files))->get();

			$user = User::with('documents','applications')->find(\Auth::id());
			foreach($files as $slug => $file)
			{
				if($document = $documents->where('slug',$slug)->first()){
					$filename=uniqid().'-'.$document->slug.'-'.$file->getClientOriginalName();
	                $file->move(
	                    base_path().'/public/uploads/'.$user->id, $filename
	                );

					$user->documents()->syncWithoutDetaching([$document->id =>['path'=>$filename]]);
				}
			}
		}
		return redirect()->route('dsu.client.user.profile');
	}
}
?>